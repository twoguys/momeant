...
module Amazon
  module FPS
    ...
    class Payments
      ...
      def self.get_postpaid_cobranded_url(user_id, unique_id, return_url)
        uri = URI.parse(ENV["AMAZON_CBUI_ENDPOINT"])
        parameters = {}
        parameters["callerKey"] = ENV["S3_KEY"]
        parameters["version"] = "2009-01-09"
        parameters["callerReferenceSender"] = user_id
        parameters["callerReferenceSettlement"] = unique_id
        parameters["globalAmountLimit"] = 20.00
        parameters["pipelineName"] = "SetupPostpaid"
        parameters["returnURL"] = return_url
        parameters["signature"] = Amazon::FPS::SignatureUtils.sign_parameters({:parameters => parameters, 
                                                :aws_secret_key => ENV["S3_SECRET"],
                                                :host => uri.host,
                                                :verb => "GET",
                                                :uri  => uri.path })
